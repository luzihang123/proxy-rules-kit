...
- 长桥、富途（Moomoo）、老虎证券等港资持牌券商，交易与行情服务器主要位于香港，默认走香港节点延迟最低、稳定性最佳；在香港节点不可用时依次回退日本、台湾、新加坡、美国。这几家券商虽未被墙，但会识别大陆 IP 并拒绝服务（提示"不服务大陆客户"），必须走大陆以外节点才能正常登录和交易。2026 年 6 月 10 日，香港证监会（SFC）明确香港持牌主体可继续服务存量内地客户（不在境内提供、且合规），预计服务需求长期存在，值得单独建组维护。
...
| `RouteKit-Broker` | 港资券商（长桥 / 富途 / 老虎证券） | 未被墙但识别大陆 IP 会拒绝服务，必须走境外节点；香港优先，回退顺序：日本→台湾→新加坡→美国。SFC 于 2026-06-10 明确持牌主体可续服务存量内地客户，需求长期存在 |
